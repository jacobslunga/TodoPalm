...
describe("Todos", () => {
  ...
  describe("GET /api/v1/todos", () => {
    test("fetches all todos", async () => {
      ...
      const response = await request(app).get("/api/v1/todos").set("Authorization", `Bearer ${mockToken}`);
  ...
  describe("GET /api/v1/todos/:id", () => {
    test("fetches todo by ID", async () => {
      ...
      const response = await request(app).get("/api/v1/todos/1").set("Authorization", `Bearer ${mockToken}`);
      expect(response.statusCode).toBe(200);
  ...
  describe("DELETE /api/v1/todos/:id", () => {
    test("deletes a todo", async () => {
      ...
      const response = await request(app).delete("/api/v1/todos/1").set("Authorization", `Bearer ${mockToken}`);
